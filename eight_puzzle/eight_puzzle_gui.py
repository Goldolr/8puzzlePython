...
from eight_puzzle.algorithm import movimientosPosibles, h
import sys
sys.setrecursionlimit(10000)

def build_gui(dimension):
    frmpuzzle = tk.Tk()
    App(frmpuzzle, dimension).pack(side='top', fill='both', expand=True)
    frmpuzzle.mainloop()
...
class App(tk.Frame):
    ...
    def start_background_task(self):
        self.t1 = Thread(target=self.msj_update)
    ...
    def define_puzzle(self):
        self.puzzle_src = Puzzle(self, {"tile_color": "red2", "text": "Combinación"})
        self.puzzle_dest = Puzzle(self, {"tile_color": "spring green4", "text": "Solución"})
    ...
    def msj_update(self):
        while True:
            ...
            if self.stop is True:
                messagebox.showinfo("Debug", "Reiniciar")
                self.reset_puzzle()
            elif res is True:
                messagebox.showinfo("Debug", "Solución encontrada")
            elif not res:
                messagebox.showinfo("Debug", "No existe una solución")
            self.done = True
    ...
    def define_display_widgets(self):
        self.label = tk.Label(self, text="8 Puzzle", font=('Verdana', 20, 'bold'))
        ...
        self.reset.pack(side=tk.LEFT, anchor=tk.N, ipadx=20, ipady=20)
        self.increase_speed = tk.Button(self, text="Aumentar", bg="dark slate gray", fg="white", command=self.inc_speed)
        self.increase_speed.pack(side=tk.RIGHT, anchor=tk.NE, ipadx=20, ipady=20)
        self.decrease_speed = tk.Button(self, text="Disminuir", bg="dark slate gray", fg="white", command=self.dec_speed)
        self.decrease_speed.pack(side=tk.RIGHT, anchor=tk.NE, ipadx=21, ipady=20)
...
class Puzzle(tk.Frame):
    ...
    def fill(self, i, j):
        self.b[i][j].config(text=self.index, state=tk.DISABLED, bg="light sea green", fg="white")
